...
export default function TextInput({ lableName, onChange, validateLength, cName }) {
    ...
    const handleTextChange = (e) => {
        ...
        let errorFlag = e.target.value.length === 0 || e.target.value.length <= validateLength

        setText(textValue)

        setTextFlag(errorFlag)

        setErrorText(textValue.length === 0 ? `The ${lableName} is required` : textValue.length <= validateLength ? `The ${lableName} length is more than ${validateLength}` : null)
