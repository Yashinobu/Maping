...
export default function CurrentMap() {
    ...
    const Map = useMemo(() => dynamic(
        () => import('@/components/Map/CurrentMap'),
        {
            ...
            <main className="flex h-screen flex-col items-center bg-white pb-2 text-[#5C5F5D]">
                <div className="fixed z-[100] h-[60px] bg-white w-full drop-shadow-lg border-b-solid border-b-[2px] border-b-[#00000029]">
                    ...
                    <div className="flex place-content-center w-full gap-2">
                        <MapPinIcon width={30} />
                        <label className="text-[#5C5F5D] font-bold text-[25px] pt-2">現在地周辺</label>
                    </div>
                </div>
                <div className="flex flex-col w-full bg-transparent px-2 mt-[60px] z-[9999] pt-7 pb-3">
                    ...
                    <div className="flex justify-center mt-5">
                        <button className="flex justify-center gap-2 bg-white text-[#5C5F5D] text-[14px] rounded-lg px-4 py-2 h-full z-[50] border-solid border-[4px] border-[#FAFB64]">すきな場所をタップしてPINをさそう！
                            <img src="./tap.svg" />
                        </button>
                    </div>
                </div>
                <div className="w-[95%] h-[100%] absolute left-[2.5%] top-[60px] xs:h-[64%] xs:top-[68px] 2xs:h-[77%] 2xs:top-[75px] sm:h-[90%] md:h-[90%] sm:top-[60px]" onClick={handleClick}>
                    <Map state={state} />
                </div>
